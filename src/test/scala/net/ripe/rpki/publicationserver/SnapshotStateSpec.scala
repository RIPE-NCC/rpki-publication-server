...
class SnapshotStateSpec extends PublicationServerBaseSpec {
  ...
  test("should serialize a SnapshotState to proper xml") {
    ...
    val state = new SnapshotState(UUID.fromString("s123"), BigInt(123), pdus)
  ...
  private val emptySnapshot = new SnapshotState(UUID.fromString("session1"), BigInt(1), Map.empty)
  ...
  test("should add an object with publish") {
    ...
    s.right.get.sessionId should be(UUID.fromString("session1"))
